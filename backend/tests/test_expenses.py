...
class TestDailyBudgetStatistics:
    ...
    def test_daily_stats_no_expenses(
        ...
        assert stats['expense_count_today'] == 0
        # All categories should be returned (with 0 spent) - this changed in recent update
        assert len(stats['by_category_today']) == 8  # 8 default categories
        # All should have 0 spent
    ...
    def test_daily_stats_category_breakdown(
        ...
        # Check category breakdown - now returns ALL categories (changed in recent update)
        assert len(stats['by_category_today']) == 8  # All default categories
