...
import { formatCurrency, getBudgetColor } from '@/lib/budget-utils';
...
/**
 * Main budget metrics card showing "Remaining Today" and "Already Spent"
 *
 * Features:
 * - Large "Remaining Today" display (main focus)
 * - Cumulative savings/overspending indicator
 * - "Already Spent" with progress bar
 * - Expense count and available percentage
 * - Responsive layout (mobile vs desktop)
 * - Color coding (green/amber/red)
 */
export const BudgetMetricsCard = forwardRef<HTMLDivElement, BudgetMetricsCardProps>(
  ({ statistics, currencyCode }, ref) => {
    ...
    const progressColor = getBudgetColor(
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {statistics.is_over_budget ? (
                      <TrendingDown className="h-3.5 w-3.5" style={{ color: progressColor }} />
                    ) : (
                      <TrendingUp className="h-3.5 w-3.5" style={{ color: progressColor }} />
                    )}
                  </div>
